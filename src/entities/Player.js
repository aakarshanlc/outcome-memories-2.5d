...
export class Player {
    constructor(scene, controls, color, charName = 'Sonic') {
        ...
        if (charName === 'Knuckles') {
            ...
            this.blockMesh.rotation.x = Math.PI / 2; // Lay flat
            this.blockMesh.visible = false;
            this.mesh.add(this.blockMesh); // Attach to player
        }
    }
    ...
    triggerBlockSuccess(killer) {
        ...
        if (killer) {
            killer.stun(this.config.abilities.parry.stunDuration);
        }
    }
    ...
    update(obstacles, killers, gameManager) {
        ...
        if (this.characterName === 'Sonic') {
            ...
            if (this.controls.ability1 && this.ability1Cooldown <= 0 && !this.isBlocking) {
                ...
                this.ability1Cooldown = this.config.abilities.parry.cooldown;
                if (this.blockMesh) this.blockMesh.visible = true; // Show ring
            }
            ...
            if (this.controls.ability1 && this.ability1Cooldown <= 0) {
                ...
                gameManager.spawnProjectile(
                    this.mesh.position.x, this.mesh.position.z, dir.x, dir.z, this, 
                    this.config.abilities.gun.damage, 
                    this.config.abilities.gun.stunDuration, 
                    this.config.abilities.gun.projectileSpeed
                );
                this.ability1Cooldown = this.config.abilities.gun.cooldown;
            }
            if (this.controls.ability2 && this.ability2Cooldown <= 0) {
                this.isFlying = true;
                this.flyTimer = this.config.abilities.fly.duration;
                this.ability2Cooldown = this.config.abilities.fly.cooldown;
            }
            if (this.isFlying) {
